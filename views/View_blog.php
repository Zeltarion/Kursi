...
<?php
 ...
 {
     $post_array = array();
     $post_array = $posts->posts_blog($_GET['tag_name'],$_GET['id_blog']);
     $tag_mas = $tags->list_tag($post_array[0]);
     ...
     for($i=0;$i<$col_post;$i++)
     {
         ...
         {
             echo '<a href=\'../controllers/Controller_blog.php?tag_name='.$tag_mas[$i][$j].'\'>'.$tag_mas[$i][$j].'</a>';
         }
     }
 }
 else
 {
     ...
     for($i=0;$i<$col_post;$i++)
     {
         ...
         for($j=0;$j<sizeof($tag_mas[$i][$j]);$j++)
         {
             echo '<a href=\'../controllers/Controller_blog.php?tag_name='.$tag_mas[$i][$j].'&id_blog='.$id_blog.'\'>'.$tag_mas[$i][$j].'</a>';
         }
     }
 }
  ...
  if(!empty($_POST['submit_create_blog']))
  {
      ...
      //var_dump($users->id_users());
      $blogs->insert(array('id_user'=>$id_users,'name'=>$_POST['name_blog'],'description'=>$_POST['description'],'date'=>$date));
  }
  ...
  if(!empty($_POST['submit_create_post']))
  {
     ...
     */
     $id_record_post = $posts->insert(array('name'=>$_POST['name_post'],'text'=>$_POST['text_post'],'path_image'=>$_POST['path_file_image_post']));
   ...
   If(!empty($_POST['input_tags']))
   {
    ...
    {
        $id_record_tag = $tags->insert(array('name'=>$tag_mas[$i]));
        $posts_tags->update(array('id_post'=>$id_record_post),array('id_tag'=>$id_record_tag));
    }
   }
  }
  ...
  { // УДАЛЕНИЕ ДАННЫХ ПОСТА ИЗ БД
    $id_tags = array();
    $id_tags = $posts_tags->id_tags($id_post);
    $posts->delete($id_post);
    $posts_tags->delete($id_post);
    $tags->delete($id_tags);
  }
  ...
  if(!empty($_POST['submit_select_blog']))
  {
    ...
    else
    {
        $post_array = array();
        $tag_mas = array();
        $id_blog = $blogs->blog_content($_POST['select_name_blog'],$users->id_users());
        $post_array = $posts->list_post($id_blog);
        $tag_mas = $tags->list_tag($post_array[0]);
